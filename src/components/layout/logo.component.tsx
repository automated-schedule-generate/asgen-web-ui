...
const CustomGridIcon = ({ size }: { size: number }) => (
  <Box 
    component="img" 
    src="/images/gerar-grade.png" 
    alt="Gerar Grades"
    sx={{ 
      width: size, 
      height: size, 
...
const dashboardCards = [
  ...
  { title: 'Gerar grades', icon: <CustomGridIcon size={35} /> },
...
const sidebarItems = [
  ...
  { label: 'Gerar grades', icon: <CustomGridIcon size={20} /> },
  { label: 'Visualização das grades', icon: <VisibilityIcon sx={{ fontSize: 22 }} /> },
];
...
export default function DashboardPage() {
  ...
  const welcomePrefix = isFemale ? 'Seja bem-vinda' : 'Seja bem-vindo';
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', bgcolor: '#e8edf5' }}>
      ...
      <Box component="header" sx={{ bgcolor: '#03017D', color: '#fff', px: 3, py: 1.2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 1100 }}>
        <Image src="/images/asgen-horizontal-light.svg" alt="ASgen" width={120} height={30} style={{ width: 'auto', height: '30px' }} />
      ...
      <Box sx={{ display: 'flex', flex: 1 }}>
        ...
        <Box component="main" sx={{ flex: 1, p: 6, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          >
            <Typography 
              variant="h6" 
              sx={{ mb: 4, fontWeight: 700, color: '#444', textAlign: 'center', fontSize: '1.3rem' }} 
            >
              Painel de controle
            </Typography>
