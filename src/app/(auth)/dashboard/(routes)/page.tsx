...
export default function DashboardPage() {
  return (
    ...
    >
      ...
      >
        <Typography variant="h6" sx={{ mb: 4, fontWeight: 700, color: '#444', textAlign: 'center' }}>
          Painel de controle
        </Typography>
        ...
        >
          {dashboardCards.map(({ title, icon }) => (
            ...
            >
              <CardActionArea sx={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', p: 2 }}>
                <Avatar sx={{ bgcolor: '#03017D', width: 55, height: 55, mb: 1.5 }}>
                  {icon}
                </Avatar>
                <Typography variant="body2" sx={{ fontWeight: 700, color: '#333', textAlign: 'center', fontSize: '0.85rem' }}>
                  {title}
                </Typography>
              </CardActionArea>
            </Card>
